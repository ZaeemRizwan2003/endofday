...
const AddFoodListing = () => {
    ...
    useEffect(() => {
        if (edit) {
            const fetchListing = async () => {
                try {
                    const res = await axios.get(`/api/getlisting?id=${edit}`);
                    setFormData({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (edit) {
                const res = await axios.put('/api/foodlisting', payload, config);
                if (res.status === 200) {
                    router.push('/Dashboard');
                }
            } else {
                const res = await axios.post('/api/foodlisting', payload, config);
                if (res.status === 200) {
                    router.push('/Dashboard');
                }
            }
