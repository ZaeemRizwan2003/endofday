...
const Dashboard = () => {
  ...
  const handleStatusChange = (orderId, newStatus, statusType) => {
    if (newStatus === "Cancelled") {
      router.push(`/Restaurants/cancel-order/${orderId}`);
    } else {
      updateOrderStatus(orderId, newStatus, statusType);
    }
  };
  ...
  const updateOrderStatus = async (orderId, newStatus, statusType) => {
    try {
      ...
      if (response.ok) {
        ...
        // Trigger restaurant status update logic if needed
        if (statusType === "deliveryStatus" && newStatus === "Picked Up") {
          updateRestaurantStatus(orderId);
        }
      } else {
  ...
  useEffect(() => {
    const updatedOrders = orders.filter(
      (order) => order.restaurantStatus !== "Done"
    );
                ...
                {filteredOrders.map((order) => (
                  ...
                  >
                    ...
                    <p className="text-lg text-gray-600">
                      Rider Contact: {order.deliveryBoy_id?.contact}
                    </p>
                    <p className="text-lg text-gray-600">
